...
export default function RepPerformancePage() {
  ...
  return (
    <div className="animate-fade-in">
      {/* Header */}
      <div className="bg-gradient-to-br from-amber-500 via-orange-500 to-rose-500 px-5 pt-5 pb-8 relative overflow-hidden">
        <div className="absolute top-0 right-0 w-40 h-40 bg-white/5 rounded-full -translate-y-1/2 translate-x-1/4" />
        <div className="absolute bottom-0 left-0 w-32 h-32 bg-white/5 rounded-full translate-y-1/2 -translate-x-1/4" />
        <div className="relative">
          <h1 className="text-white text-xl font-bold">My Performance</h1>
          <p className="text-orange-200 text-sm mt-0.5">Track your achievements</p>
        </div>
      </div>
        ...
        {performance && (
          <div className="grid grid-cols-2 gap-2.5 stagger-children">
            {[
              ...
              { label: 'Orders', value: performance.totalOrders, icon: ShoppingCart, gradient: 'from-blue-500 to-indigo-600', shadow: 'shadow-blue-500/20' },
              { label: 'Customers', value: performance.totalCustomers, icon: Users, gradient: 'from-violet-500 to-purple-600', shadow: 'shadow-violet-500/20' },
              { label: 'Target', value: `${performance.achievementPercentage.toFixed(0)}%`, icon: Target, gradient: 'from-amber-500 to-orange-600', shadow: 'shadow-amber-500/20' },
            ].map((stat) => (
                ...
                <div key={target.id} className="bg-slate-50 rounded-xl p-3.5">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${
                      target.status === 'Achieved' ? 'bg-emerald-100 text-emerald-700' : 'bg-blue-100 text-blue-700'
                    }`}>{target.status}</span>
                  </div>
