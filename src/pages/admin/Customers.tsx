...
export default function AdminCustomers() {
  ...
  const showCoordinatorCompactColumns = isCoordinatorView;
  ...
  const { data: filters } = useQuery({
    ...
    queryFn: () => (isCoordinatorView ? customersApi.coordinatorGetFilterOptions() : customersApi.adminGetFilterOptions()).then(r => r.data.data),
  });
  ...
  const { data, isLoading } = useQuery({
    queryKey: [isCoordinatorView ? 'coordinator-customers-admin-view' : 'admin-customers', page, search, sortBy, sortOrder, statusFilter, coordinatorFilter, regionFilter, subRegionFilter],
    queryFn: () => (isCoordinatorView ? customersApi.coordinatorGetAll : customersApi.adminGetAll)({
        page, pageSize: 20,
  ...
  const coordinatorSubRegions = useMemo(() => {
    if (!filters?.subRegions) return [];
    return filters.subRegions;
  }, [filters]);
  ...
  const getCustomerTaxLabel = (customer: any): string => {
    const rawType = customer?.customerType ?? customer?.taxType;
    if (typeof rawType === 'string') {
      const normalized = rawType.trim().toLowerCase();
      if (normalized === 'tax') return 'Tax';
      if (normalized === 'nontax' || normalized === 'non tax' || normalized === 'non-tax') return 'Non Tax';
      return rawType;
    }

    if (typeof customer?.isTaxCustomer === 'boolean') return customer.isTaxCustomer ? 'Tax' : 'Non Tax';
    if (typeof customer?.isTaxRegistered === 'boolean') return customer.isTaxRegistered ? 'Tax' : 'Non Tax';
  ...
  const handleRowClick = (e: React.MouseEvent, c: any): void => {
    ...
    if (isDesktop) navigate(`${isCoordinatorView ? '/coordinator/customers' : '/admin/customers'}/${c.id}`);
  ...
  const exportCustomers = async (format: 'excel' | 'pdf', onlySelected = false) => {
    ...
    if (format === 'excel') {
      ...
      const doc = new jsPDF();
      autoTable(doc, {
        head: [['Shop Name', 'Tax Type', 'Region', 'Sub-Region', 'Coordinator', 'Rep', 'Orders', 'Order Value', 'Status']],
        body: items.map((c: any) => [c.shopName, getCustomerTaxLabel(c), c.regionName || '', c.subRegionName || '', c.assignedCoordinatorName || '', c.assignedRepName || '', c.totalOrders || 0, formatCurrency(c.totalOrderValue || 0), c.approvalStatus || (c.isActive ? 'Active' : 'Inactive')]),
        styles: { fontSize: 8 },
        startY: 20,
      });
  ...
  const hasActiveFilters = !!(statusFilter || search || coordinatorFilter || regionFilter || subRegionFilter);
  ...
  return (
    <div className="animate-fade-in flex flex-col gap-4 pb-28">
      <div>
        ...
        <p className="text-slate-500 text-sm mt-1">{isCoordinatorView ? 'View customer accounts in your assigned region' : 'Manage customer accounts and registration requests'}</p>
      </div>
      ...
      <div className="sticky top-0 z-30">
        <div className="bg-white/90 backdrop-blur-md border border-slate-200/70 rounded-2xl shadow-[0_2px_16px_-4px_rgba(0,0,0,0.10)] px-4 py-3 space-y-2.5">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <select value={subRegionFilter} onChange={e => { setSubRegionFilter(e.target.value); setPage(1); }} disabled={!isCoordinatorView && !regionFilter} className="px-3 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-xs text-slate-700 outline-none focus:ring-2 focus:ring-indigo-500/15 focus:border-indigo-300 transition cursor-pointer">
              <option value="">All Sub-regions</option>
              {(isCoordinatorView ? coordinatorSubRegions : filteredSubRegions).map(sr => <option key={sr.id} value={sr.id}>{sr.name}</option>)}
            </select>
            {!isCoordinatorView && (
          ...
          renderTile={(c: any) => (
            <div className="p-4">
              ...
              <div className="flex items-center gap-3 text-xs text-slate-400 mb-3">
                {!showCoordinatorCompactColumns && c.regionName && <span className="px-2 py-0.5 rounded-full bg-indigo-50 text-indigo-600 font-medium">{c.regionName}</span>}
        ...
        <BottomSheet open={true} onClose={() => setSelectedCustomer(null)} title={selectedCustomer.shopName}>
          <div className="p-5 space-y-4">
            <div className="space-y-3 text-sm">
              ...
              <div className="flex justify-between"><span className="text-slate-500">Tax Type</span><span className="font-medium">{getCustomerTaxLabel(selectedCustomer)}</span></div>
            ...
            <button onClick={() => { navigate(`${isCoordinatorView ? '/coordinator/customers' : '/admin/customers'}/${selectedCustomer.id}`); setSelectedCustomer(null); }} className="w-full py-2.5 bg-indigo-600 text-white rounded-xl text-sm font-medium">View Full Details</button>
          </div>
        </BottomSheet>
      )}
